...
Your task is to determine if the person in the provided live image is a biometric match to any of the registered members' profile photos. You must be very critical and look for subtle differences. Do not be fooled by similar hair, glasses, or general appearance.
...
- Low Confidence (< 0.75): If you have any significant doubt, the faces are not a clear match, the image quality is poor, or they simply look like two different people, you MUST return a confidence score below 0.75.

Output Rules:
- If you find a match (confidence >= 0.75), set 'matchFound' to true, provide the 'fullName' of the matched member, and set the 'confidence' score.
- If you do not find a clear match (confidence < 0.75), you MUST set 'matchFound' to false and set the 'confidence' score. Do not provide a 'fullName'.
- If confidence is low (< 0.75), you MUST provide a brief, user-friendly 'reason' (e.g., "Low biometric similarity," "Poor lighting," "Face partially obscured").
